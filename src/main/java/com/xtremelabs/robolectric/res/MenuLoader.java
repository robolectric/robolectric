...
public class MenuLoader extends XmlLoader {
	...
	protected void processResourceXml(File xmlFile, Document document,
		...
		MenuNode topLevelNode = new MenuNode("top-level",
				new HashMap<String, String>());

		NodeList items = document.getChildNodes();
		if (items.getLength() != 1)
			throw new RuntimeException(
					"Expected only one top-level item in menu file "
							+ xmlFile.getName());
		if (items.item(0).getNodeName().compareTo("menu") != 0)
			throw new RuntimeException(
					"Expected a top-level item called 'menu' in menu file "
							+ xmlFile.getName());

		processChildren(items.item(0).getChildNodes(), topLevelNode);
		menuNodesByMenuName.put(
				"menu/" + xmlFile.getName().replace(".xml", ""), topLevelNode);
	}
	...
	private void processNode(Node node, MenuNode parent) {
		...
		if (!name.startsWith("#")) {
			...
			System.out.println("Menunode name " + name + " "
					+ menuNode.getChildren().size());
		}
	}
		...
		private void addChildrenInGroup(MenuNode source, int groupId, Menu root) {
			for (MenuNode child : source.children) {
				...
				if (name.equals("item")) {
					...
					int newGroupId = attributes.getAttributeResourceValue(
							"android", "id", 0);
