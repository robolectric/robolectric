...
public class ContentProviderOperationBuilderTest {
	...
	public void withValue() {
		builder.withValue("stringTest", "bar").withValue("intTest", 5)
	...
	public void withSelection() {
		builder.withSelection("first", new String[] { "a", "b" })
				.withSelection("second", new String[] { "c", "d" });

		final Map<String, String[]> selections = shadowBuilder.getSelections();
	...
	public void withValueBackReference() {
		...
		final int backReference = shadowBuilder
				.getWithValueBackReference("foo");
		assertThat(backReference, is(5));
	}

	@Test
	public void build() {
		final ContentProviderOperation operation = builder.build();
