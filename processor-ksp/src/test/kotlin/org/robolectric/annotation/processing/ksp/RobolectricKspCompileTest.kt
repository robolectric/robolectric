...
class RobolectricKspCompileTest {
  ...
  @Test
  fun isInAndroidSdkFalse_registersShadowButGeneratesNoShadowOf() {
    val source =
      SourceFile.kotlin(
        "ShadowHidden.kt",
        """
    ...
    val content = result.shadowsJavaContent()
    // Shadow is still registered despite isInAndroidSdk = false (className unresolvable -> raw
    // key).
    assertThat(content).contains("com.hidden.InternalClass")
    assertThat(content).contains("com.test.shadows.ShadowHidden")
    // ...but no shadowOf() helper is generated for a non-SDK shadow.
    assertThat(content).doesNotContain("shadowOf(")
  }

  @Test
  fun classNameNestedTarget_isRegisteredUnderCanonicalName() {
    // Java sources: '$' in a Java string literal is taken literally (Kotlin would interpolate it).
    val target =
      SourceFile.java(
        "Outer.java",
        """
    ...
    val shadow =
      SourceFile.java(
        ...
        // className in binary ('$') form; the processor registers the canonical (dot) name.
        @Implements(className = "com.test.shadows.Outer${'$'}Inner", isInAndroidSdk = false)
        public class ShadowNested {}
        """,
      )

    val result = compile(target, shadow)
    assertThat(result.exitCode).isEqualTo(KotlinCompilation.ExitCode.OK)
    ...
    assertThat(content)
      .contains(
        "SHADOWS.add(new AbstractMap.SimpleImmutableEntry<>(" +
          "\"com.test.shadows.Outer.Inner\", \"com.test.shadows.ShadowNested\"))"
      )
    assertThat(content).doesNotContain("com.test.shadows.Outer\$Inner")
  }
  ...
  @Test
  fun nestedClass_shouldRegisterUnderCanonicalName() {
    val source =
      SourceFile.kotlin(
        "Outer.kt",
        """
    ...
    val content = result.shadowsJavaContent()
    // SHADOWS is keyed by the canonical (dot) name, which ShadowMap looks up via
    // getCanonicalName().
    assertThat(content)
      .contains(
        "SHADOWS.add(new AbstractMap.SimpleImmutableEntry<>(" +
          "\"com.test.shadows.Outer.Inner\", \"com.test.shadows.ShadowInner\"))"
      )
    assertThat(content).doesNotContain("com.test.shadows.Outer\$Inner")
  }
