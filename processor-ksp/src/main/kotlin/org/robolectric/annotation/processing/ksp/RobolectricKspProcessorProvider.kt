...
// the javac processor's RobolectricProcessor); the literal values must not change.
...
private class RobolectricKspProcessor(private val environment: SymbolProcessorEnvironment) :
  ...
  override fun process(resolver: Resolver): List<KSAnnotated> {
    // The javac processor also throws when this required option is missing.
    val shadowPackage =
      requireNotNull(environment.options[PACKAGE_OPT]?.ifBlank { null }) {
        "no package specified for $PACKAGE_OPT"
      }

    val shouldInstrumentPackages =
      environment.options[SHOULD_INSTRUMENT_PKG_OPT]?.toBoolean() ?: true
