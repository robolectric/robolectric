...
class ResTableConfigTest {
    ...
    fun  isBetterThan_language_comparedNotSame_requestedEnglishUS(){
            // When requested has no language, is not a better match
            Truth.assertThat(newBuilder().setLanguage(LANGUAGE_FRENCH).build()
                    .isBetterThan(newBuilder().setLanguage(LANGUAGE_SPANISH).build(), newBuilder().setLanguage(
                            ResTable_config.kEnglish).build()))
                    .isTrue()
        }
    ...
    public class ResTableConfigBuilder {
        ...
        fun build(): ResTable_config {
            return ResTable_config(0, mcc, mnc, language, region, orientation, touchscreen, density, keyboard, navigation, inputFlags, screenWidth,
