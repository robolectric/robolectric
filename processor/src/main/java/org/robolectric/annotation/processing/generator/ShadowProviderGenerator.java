...
public class ShadowProviderGenerator extends Generator {
  ...
  void generate(PrintWriter writer) {
    ...
    writer.println("  public void reset(org.robolectric.internal.ClassTracker classTracker) {");
    for (RobolectricModel.ResetterInfo resetterInfo : model.getResetters()) {
      ...
      String shadowClass = resetterInfo.getShadowClassName();
      String actualBinaryName = resetterInfo.getActualBinaryName();
      writer.println(
          "    if (classTracker.isClassLoaded(\""
              + shadowClass
              + "\") || classTracker.isClassLoaded(\""
              + actualBinaryName
              + "\")) {");
