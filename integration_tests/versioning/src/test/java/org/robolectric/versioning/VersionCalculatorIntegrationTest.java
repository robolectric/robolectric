...
public final class VersionCalculatorIntegrationTest {
  ...
  public void sdksMatchSdkProvider() throws IOException {
    ...
    for (Sdk sdk : provider.getSdks()) {
      SdkInfo sdkFromVersions =
          new VersionCalculator().calculateSdkInfo(new JarFile(sdk.getJarPath().toFile()));
      assertThat(sdkFromVersions.apiLevel).isEqualTo(sdk.getApiLevel());
    }
  }
}
