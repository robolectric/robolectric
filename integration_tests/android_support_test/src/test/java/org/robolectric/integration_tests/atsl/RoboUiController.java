...
public class RoboUiController implements android.support.test.espresso.UiController {
  ...
  public boolean injectMotionEvent(MotionEvent event)
      throws android.support.test.espresso.InjectEventSecurityException {
    loopMainThreadUntilIdle();

    WindowManagerGlobal wmGlobal = WindowManagerGlobal.getInstance();
    ArrayList<ViewRootImpl> viewRoots =
        org.robolectric.util.ReflectionHelpers.getField(wmGlobal, "mRoots");
  ...
  public void loopMainThreadForAtLeast(long millisDelay) {
    org.robolectric.shadows.ShadowApplication.getInstance()
        .getForegroundThreadScheduler()
        .advanceBy(millisDelay, TimeUnit.MILLISECONDS);
  }
}
