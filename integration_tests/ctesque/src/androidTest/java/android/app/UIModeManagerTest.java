    ...
    try (ActivityScenario<TestActivity> scenario = ActivityScenario.launch(TestActivity.class)) {
      scenario.onActivity(
          activity -> {
            ...
            // First, reset two instances with same value.
            applicationUiModeManager.setApplicationNightMode(UiModeManager.MODE_NIGHT_NO);
            activityUiModeManager.setApplicationNightMode(UiModeManager.MODE_NIGHT_NO);
            assertThat(applicationUiModeManager.getNightMode())
                .isEqualTo(UiModeManager.MODE_NIGHT_NO);
            assertThat(activityUiModeManager.getNightMode()).isEqualTo(UiModeManager.MODE_NIGHT_NO);

            // Second, update night mode through UiModeManager got from Activity context.
            activityUiModeManager.setApplicationNightMode(UiModeManager.MODE_NIGHT_YES);

            assertThat(activityUiModeManager.getNightMode())
