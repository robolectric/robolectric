package org.robolectric.rap.ksp;

import static android.os.Build.VERSION_CODES.P;
import static android.os.Build.VERSION_CODES.S;
import static com.google.common.truth.Truth.assertThat;
import static org.robolectric.Shadows.shadowOf;

import org.junit.Test;
import org.junit.runner.RunWith;
import org.robolectric.RobolectricTestRunner;
import org.robolectric.RuntimeEnvironment;
import org.robolectric.annotation.Config;
import org.robolectric.shadow.api.Shadow;

@RunWith(RobolectricTestRunner.class)
public final class ExtendedShadowApplicationTest {
  @Config(sdk = S)
  @Test
  public void shadowIsExtracted_atMaxSdk() {
    ExtendedShadowApplication application = Shadow.extract(RuntimeEnvironment.getApplication());
    assertThat(application).isNotNull();
  }

  @Config(sdk = P)
  @Test
  public void shadowIsExtracted_atMinSdk() {
    ExtendedShadowApplication application = Shadow.extract(RuntimeEnvironment.getApplication());
    assertThat(application).isNotNull();
  }

  @Config(sdk = S)
  @Test
  public void builtInShadowOf_returnsCustomShadow() {
    assertThat(shadowOf(RuntimeEnvironment.getApplication()))
        .isInstanceOf(ExtendedShadowApplication.class);
  }
}
