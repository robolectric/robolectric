...
if [[ $# -eq 1 ]]; then
  CL=$1
  /google/data/ro/teams/copybara/copybara copy.bara.sky generate-piper-diff-from-cl $CL --force --dry-run --git-destination-path $local_git_folder
else
  /google/data/ro/teams/copybara/copybara copy.bara.sky generate-piper-diff --force --dry-run --git-destination-path $local_git_folder
fi
