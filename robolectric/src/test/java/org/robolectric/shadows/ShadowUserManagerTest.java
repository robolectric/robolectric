...
public class ShadowUserManagerTest {
  ...
  @Test
  @Config(minSdk = N)
  public void switchToProfileShouldThrowException() {
    try {
      shadowOf(userManager)
          .addProfile(10, 11, "profile_user", ShadowUserManager.FLAG_MANAGED_PROFILE);
      shadowOf(userManager).switchUser(11);
      fail("Switching to profile should throw UnsupportedOperationException");
    } catch (UnsupportedOperationException e) {
      assertThat(e).hasMessageThat().isEqualTo("Cannot switch to profile");
    }
  }
