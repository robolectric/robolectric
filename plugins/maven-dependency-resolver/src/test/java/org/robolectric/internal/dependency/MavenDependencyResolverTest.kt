...
class MavenDependencyResolverTest {
    ...
    fun setUp() {
        ...
        mavenArtifactFetcher = TestMavenArtifactFetcher(
                REPOSITORY_URL,
                REPOSITORY_USERNAME,
                REPOSITORY_PASSWORD,
                localRepositoryDir,
    ...
    @Throws(Exception::class)
    @Test
    fun getLocalArtifactUrl_placesFilesCorrectlyForSingleURL() {
    ...
    private fun checkJarArtifact(artifact: MavenJarArtifact) {
        ...
        val jarSha1 = File(localRepositoryDir, artifact.jarSha1Path())
        val pomSha1 = File(localRepositoryDir, artifact.pomSha1Path())
        ...
        assertThat(readFile(jarSha1)).isEqualTo(sha1("$artifact jar contents"))
        assertThat(pom.exists()).isTrue()
        assertThat(readFile(pomSha1)).isEqualTo(sha1("$artifact pom contents"))
    }
    ...
    fun localArtifactUrl_doesNotFetchWhenArtifactsExist() {
            ...
            Truth.assertThat(artifactFile.exists()).isTrue()
            mavenDependencyResolver!!.getLocalArtifactUrl(dependencyJar)
            Truth.assertThat(mavenArtifactFetcher!!.numRequests).isEqualTo(0)
        }
    ...
    fun localArtifactUrl_handlesFileNotFound() {
            ...
            Assert.assertThrows(
                    AssertionError::class.java) { mavenDependencyResolver!!.getLocalArtifactUrl(dependencyJar) }
        }

    @Throws(Exception::class)
    @Test
    fun localArtifactUrl_handlesInvalidSha1() {
            val dependencyJar = DependencyJar("group", "artifact-invalid-sha1", "1")
    ...
    internal inner class TestMavenDependencyResolver : MavenDependencyResolver() {
        override fun createMavenFetcher(
                ...
                executorService: ExecutorService): MavenArtifactFetcher {
        ...
        override fun getLocalRepositoryDir(): File {
    ...
    internal class TestMavenArtifactFetcher(
            ...
            repositoryPassword: String,
            localRepositoryDir: File,
            private val executorService: ExecutorService) : MavenArtifactFetcher(
            repositoryUrl,
            repositoryUserName,
            repositoryPassword,
            localRepositoryDir,
        ...
        override fun createFetchToFileTask(remoteUrl: URL, tempFile: File): ListenableFuture<Void> {
            return Futures.submitAsync(
                    object : FetchToFileTask(remoteUrl, tempFile, null, null) {
                        @Throws(Exception::class)
    ...
    companion object {
        private lateinit var REPOSITORY_DIR: File
        private lateinit var REPOSITORY_URL: String
        private const val REPOSITORY_USERNAME = "username"
        private const val REPOSITORY_PASSWORD = "password"
        private val SHA1 = Hashing.sha1()

        private val successCases = arrayOf(
        ...
        fun addTestArtifact(dependencyJar: DependencyJar?) {
            ...
            try {
                ...
                Files.write(
                        sha1(jarContents).toByteArray(), File(REPOSITORY_DIR, mavenJarArtifact.jarSha1Path()))
                ...
                Files.write(
                        sha1(pomContents).toByteArray(), File(REPOSITORY_DIR, mavenJarArtifact.pomSha1Path()))
        ...
        @Throws(IOException::class)
        fun addTestArtifactInvalidSha1(dependencyJar: DependencyJar?) {
            val mavenJarArtifact = MavenJarArtifact(dependencyJar)
            try {
                Files.createParentDirs(File(REPOSITORY_DIR, mavenJarArtifact.jarPath()))
                val jarContents = "$mavenJarArtifact jar contents"
                Files.write(jarContents.toByteArray(), File(REPOSITORY_DIR, mavenJarArtifact.jarPath()))
                Files.write(
                        sha1("No the same content").toByteArray(),
                        File(REPOSITORY_DIR, mavenJarArtifact.jarSha1Path()))
                val pomContents = "$mavenJarArtifact pom contents"
                Files.write(pomContents.toByteArray(), File(REPOSITORY_DIR, mavenJarArtifact.pomPath()))
                Files.write(
                        sha1("Really not the same content").toByteArray(),
                        File(REPOSITORY_DIR, mavenJarArtifact.pomSha1Path()))
            } catch (e: MalformedURLException) {
                throw AssertionError(e)
            }
        }

        fun sha1(contents: String?): String {
            return SHA1.hashString(contents, StandardCharsets.UTF_8).toString()
        }

        @Throws(IOException::class)
        fun readFile(file: File?): String {
            return String(Files.asByteSource(file).read())
        }
    }
}