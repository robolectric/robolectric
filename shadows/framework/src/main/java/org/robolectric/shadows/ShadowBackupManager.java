 ...
 * transport and performing restores, stores which packages are restored from which backup set,
 * and can be verified using methods on the shadow like {@link #getPackageRestoreToken(String)}.
 */
...
public class ShadowBackupManager {
  ...
  private long lastRestoreToken = 0L;
  private final Map<Long, List<String>> restoreData = new HashMap<>();
  private final Map<String, Long> restoredPackages = new HashMap<>();
  ...
  public long getPackageRestoreToken(String packageName) {
    Long token = restoredPackages.get(packageName);
  ...
  public void addAvailableRestoreSets(long restoreToken, List<String> packages) {
    restoreData.put(restoreToken, packages);
  }
  ...
  private class FakeRestoreSession implements IRestoreSession {
    ...
    public int getAvailableRestoreSets(IRestoreObserver observer, IBackupManagerMonitor monitor)
      ...
      post(
          () -> {
            Set<Long> restoreTokens = restoreData.keySet();
    ...
    public int restoreSome(
        ...
        throws RemoteException {
      List<String> restorePackages = new ArrayList<>(restoreData.get(token));
      if (packages != null) {
      ...
      for (int i = 0; i < restorePackages.size(); i++) {
        ...
        restoredPackages.put(restorePackages.get(index), token);
      }
    ...
    public int restorePackage(
        ...
        throws RemoteException {
      if (lastRestoreToken == 0L) {
        return -1;
      }
      List<String> restorePackages = restoreData.get(lastRestoreToken);
      ...
      restoredPackages.put(packageName, lastRestoreToken);
      post(() -> observer.restoreFinished(BackupManager.SUCCESS));
      return BackupManager.SUCCESS;
    }
