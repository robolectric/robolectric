...
public class ShadowBluetoothLeScanner {
  ...
  protected void startScan(List<ScanFilter> filters, ScanSettings settings, ScanCallback callback) {
    scanCallbacks.put(callback, new ScanParams(unmodifiableList(filters), settings));
  }
  ...
  protected int startScan(
    ...
    pendingIntents.put(pendingIntent, new ScanParams(unmodifiableList(filters), settings));
    return 0;
  }
