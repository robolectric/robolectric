...
public class ShadowContextHubTransaction<T> {
  ...
  private static final AtomicBoolean shouldThrowTimeout = new AtomicBoolean(false);
  ...
  interface ContextHubTransactionReflector {
    @Direct
    <R> ContextHubTransaction.Response<R> waitForResponse(long timeout, TimeUnit unit);
  }
}
