...
class RobolectricJupiterEngine : TestEngine {
  ...
  private fun hasClassLifecycleMethods(testClass: Class<*>): Boolean {
    return AnnotationSupport.findAnnotatedMethods(
        testClass,
  ...
  private fun selectMethodSdkWithFallback(
    ...
    val selectedSdks = deps.sdkPicker.selectSdks(methodConfiguration, methodManifest)
    val selectedSdk = selectedSdks.firstOrNull()
    if (selectedSdk != null) return selectedSdk

    val sdkCollection = resolveSdkCollection(deps, testClass, method)
    return sdkCollection?.let { createFallbackSdk(it, testClass, method, methodConfig) }
  }

  private fun resolveSdkCollection(
    deps: RobolectricDependencies,
    testClass: Class<*>,
    method: Method,
  ): Any? {
    return try {
      ...
      RunnerLogger.error(
        "SdkCollection class was not found while resolving fallback SDK for ${testClass.name}.${method.name}",
        e,
      )
      null
    }
  ...
  private fun createFallbackSdk(
    ...
    if (methodConfig.sdk.isNotEmpty()) {
      val getSdkMethod = sdkCollection.javaClass.getMethod("getSdk", Int::class.javaPrimitiveType)
      val fallbackSdk = getSdkMethod.invoke(sdkCollection, methodConfig.sdk[0]) as Sdk
      ...
      RunnerLogger.logSdkFallback(testClass.simpleName, method.name, fallbackSdk.apiLevel, reason)
      System.err.println(
        "WARNING: System property 'robolectric.enabledSdks=$enabledSdksProperty' " +
          ...
          "Configured SDKs: ${methodConfig.sdk.joinToString(",")}. " +
  ...
  private fun runTestInSandbox(sandbox: AndroidSandbox, testClass: Class<*>, method: Method) {
    ...
    try {
      invokeTestMethod(bootstrappedTestClass, testInstance, method, sandbox)
    } finally {
      ...
      try {
        ...
        RunnerLogger.error("Error in @AfterEach for ${testClass.simpleName}.${method.name}", e)
      }
    }
  }
