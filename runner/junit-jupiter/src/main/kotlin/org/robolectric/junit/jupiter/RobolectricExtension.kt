 ...
 *
 * The Jupiter integration ships two independent entry points that each own the Robolectric class
 * lifecycle:
 * - This extension (via `@ExtendWith`), driven by Jupiter's callback/interceptor model.
 * - [RobolectricJupiterEngine] (auto service-loaded), a full [TestEngine] that replaces the default
 *   Jupiter engine.
 *
 * They are **not** designed to be active at the same time on the same test class. Do not register
 * this extension on a test run that is executed by [RobolectricJupiterEngine] — doing so would
 * cause both to attempt class-level sandbox setup, producing duplicate environment bootstraps and
 * unpredictable teardown ordering. Pick one path per run.
 */
...
class RobolectricExtension :
  ...
  private fun <T : Any> Store.getOrCreate(
    ...
    return computeIfAbsent(key, { defaultCreator() }, requiredType)
  }
  ...
  private fun getOrCreateDependencies(context: ExtensionContext): RobolectricDependencies {
    return context.root.getStore(NAMESPACE).getOrCreate(
      DEPENDENCIES_KEY,
  ...
  override fun beforeEach(context: ExtensionContext) {
    ...
    context.getStore(NAMESPACE).getOrCreate(CONTROLLERS_KEY, ControllerTracker::class.java) {
      ControllerTracker()
    }
  }
