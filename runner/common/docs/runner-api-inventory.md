...
data class RobolectricDependencies(
  ...
  val frameworkClassLoadingBridge: FrameworkClassLoadingBridge,
)
```
...
|--------|------------|---------|---------------|
| `create()` | `properties: Properties = System.getProperties(), frameworkClassLoadingBridge: FrameworkClassLoadingBridge = DefaultFrameworkClassLoadingBridge` | `RobolectricDependencies` | Thread-safe |
...
| Method | Parameters | Returns | Thread Safety |
|--------|------------|---------|---------------|
| `createSandbox()` | `testClass: Class<*>, testMethod: Method? = null, sourceClassLoader: ClassLoader? = testClass.classLoader` | `SandboxContext` | Not thread-safe |
| `createSandboxes()` | `testClass: Class<*>, testMethod: Method? = null, sourceClassLoader: ClassLoader? = testClass.classLoader` | `List<SandboxContext>` | Not thread-safe |
...
**Behavior Note**:
- `createSandboxes()` fails fast when SDK contexts cannot be resolved and includes diagnostic context (for example reflection fallback failures), rather than returning an empty result.
...
| Method | Parameters | Returns | Thread Safety |
|--------|------------|---------|---------------|
| `setupForClass()` | `testClass: Class<*>, sourceClassLoader: ClassLoader? = testClass.classLoader` | `SandboxContext` | Thread-safe (uses ConcurrentHashMap) |
| `tearDownForClass()` | `testClass: Class<*>` | `Unit` | Thread-safe |
| `getClassContext()` | `testClass: Class<*>` | `SandboxContext?` | Thread-safe |
| `executeInClassContext()` | `testClass: Class<*>, testName: String, configuration: Configuration? = null, block: () -> T` | `T` | Must call from proper thread |

**Behavior Note**:
- If `configuration == null`, execution uses the persistent class environment.
- If `configuration != null`, execution uses an isolated method environment while reusing the class-level sandbox/classloader.

---

### FrameworkClassLoadingBridge

**Purpose**: Bridge for framework-provided classloaders and transformed class bytes.
...
Use this API when framework test classes are transformed/loaded by custom classloaders.
...
sealed class ExecutionResult {
  data class Success(val durationMs: Long) : ExecutionResult()
  data class Failure(val error: Throwable, val durationMs: Long = 0) : ExecutionResult()
...
- SDK/sandbox discovery failures are surfaced explicitly (fail-fast) rather than silently skipped
