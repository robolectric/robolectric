...
- ✅ **Fail-fast discovery diagnostics** - SDK/sandbox resolution failures are surfaced explicitly
- ✅ **Custom classloader bridge** - Support framework-transformed test classes (e.g., Kotest-style loaders)
...
val integration = RobolectricIntegrationBuilder
  ...
  .classLoadingBridge(MyFrameworkClassLoadingBridge) // optional for custom loaders
  .enableDebugLogging()
  .build()
...
// val deps = RobolectricDependencies.create(
//   frameworkClassLoadingBridge = MyFrameworkClassLoadingBridge
// )
...
│                        runner:common Core                            │
│  ┌──────────────────────┐  ┌──────────────────────┐                 │
│  │ SandboxLifecycleManager│ │ ClassLifecycleManager │               │
│  └──────────────────────┘  └──────────────────────┘                 │
│  ┌──────────────────────┐  ┌──────────────────────┐                 │
│  │ RobolectricSandboxExecutor │ │ TestBootstrapper │                │
│  └──────────────────────┘  └──────────────────────┘                 │
│  ┌──────────────────────┐  ┌──────────────────────┐                 │
│  │ LifecycleHelper      │  │ DiscoveryHelpers     │                 │
│  └──────────────────────┘  └──────────────────────┘                 │
│  ┌──────────────────────┐  ┌──────────────────────┐                 │
│  │ ParameterResolver    │  │ RunnerLogger/Metrics │                 │
│  └──────────────────────┘  └──────────────────────┘                 │
└─────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
...
`createSandboxes()` is used for multi-SDK discovery/execution. If SDK contexts cannot be resolved,
it throws with detailed diagnostics (including fallback reflection failures) instead of silently
dropping tests.

#### ClassLifecycleManager
Manages class-level lifecycle context (`@BeforeAll`/`@AfterAll`) with two execution modes:

- **Persistent class environment** when `configuration == null`
- **Isolated per-method environment** when method configuration is provided

This allows class-level sandbox reuse while still honoring method-level configuration overrides.
#### TestBootstrapper
...
4. Discovery logs for explicit SDK resolution errors (the runner now fails fast on discovery errors)
...
```
## See Also
- [Runner API Inventory](docs/runner-api-inventory.md)
- [Runner Integration Matrix](docs/runner-integration-matrix.md)
