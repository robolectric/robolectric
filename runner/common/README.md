...
This module provides shared infrastructure for building Robolectric test runners for any testing framework.
## Quick Start
...
class MyTestEngine : TestEngine {
  override fun discover(request: EngineDiscoveryRequest, uniqueId: UniqueId): TestDescriptor {
    ...
    // Use DiscoveryHelpers for test discovery
    request.getSelectorsByType(ClassSelector::class.java).forEach { selector ->
      ...
      // Create descriptors for each method
