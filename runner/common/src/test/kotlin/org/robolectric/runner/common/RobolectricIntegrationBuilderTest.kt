...
class RobolectricIntegrationBuilderTest {
  ...
  fun `parameterResolver can be customized`() {
    val customResolver =
      object : ParameterResolver {
        override fun resolveParameter(parameter: Parameter, sandbox: AndroidSandbox): Any? = null
      }
  ...
  @Test
  fun `classLoadingBridge can be configured`() {
    val bridge = object : FrameworkClassLoadingBridge {}
    val integration =
      RobolectricIntegrationBuilder().classLoadingBridge(bridge).build()
        as DefaultRobolectricIntegration

    assertThat(integration.getLifecycleManager()).isNotNull()
  }
