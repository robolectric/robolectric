...
class RobolectricDependenciesTest {
  ...
  @Test
  fun `create keeps custom framework class loading bridge`() {
    val bridge = object : FrameworkClassLoadingBridge {}

    val deps = RobolectricDependencies.create(frameworkClassLoadingBridge = bridge)

    assertThat(deps.frameworkClassLoadingBridge).isSameInstanceAs(bridge)
  }
}
