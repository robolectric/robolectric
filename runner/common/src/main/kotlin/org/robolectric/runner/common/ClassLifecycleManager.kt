...
class ClassLifecycleManager(private val sandboxLifecycleManager: SandboxLifecycleManager) {
  ...
  /**
   * Combined class-level state. Keeping context and environment in one record makes setup/teardown
   * atomic and removes the old risk of the two maps drifting out of sync.
   */
  private data class ClassState(
    val context: SandboxLifecycleManager.SandboxContext,
    val environment: RobolectricEnvironment,
  )

  private val classStates = ConcurrentHashMap<Class<*>, ClassState>()
  ...
  fun setupForClass(
    ...
    classStates[testClass]?.let { existing ->
    ...
    val context =
      RunnerMetrics.timed(RunnerMetrics.PHASE_CLASS_SETUP) {
        sandboxLifecycleManager.createSandbox(
          testClass,
          testMethod = null,
    ...
    classStates[testClass] = ClassState(context, environment)
  ...
  fun tearDownForClass(testClass: Class<*>) {
    val state = classStates.remove(testClass) ?: return
    RunnerMetrics.timed(RunnerMetrics.PHASE_CLASS_TEARDOWN) {
      state.context.sandbox.runOnMainThread {
        state.environment.access { state.environment.tearDownApplicationState() }
      }
    }
  ...
  fun getClassContext(testClass: Class<*>): SandboxLifecycleManager.SandboxContext? {
    val state = classStates[testClass] ?: return null
    RunnerLogger.logSandboxReuse(testClass.simpleName, state.context.sdk.apiLevel)
   ...
   * @param configuration Optional method-level configuration. If null, execution uses the
   *   persistent class environment. If non-null, execution uses an isolated method environment
   *   while reusing the class-level sandbox/classloader.
  ...
  fun <T> executeInClassContext(
    ...
    val state = classStates[testClass]
    check(state != null) {
      "No class context found for $testClass. Did you call setupForClass() first?"
    }

    try {
      return state.context.sandbox.runOnMainThread(
        Callable {
          if (configuration == null) {
            state.environment.access(block)
          } else {
            // Method-level configuration requires a fresh per-method environment for isolation,
            // while still reusing the class-level sandbox/classloader.
