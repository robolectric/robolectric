...
class SandboxLifecycleManager(private val dependencies: RobolectricDependencies) {
  ...
  fun createSandboxes(
    ...
    var fallbackReflectionFailure: Throwable? = null
    if (sdks.isEmpty()) {
    ...
    if (sdks.isEmpty()) {
      val reflectionDetails =
        fallbackReflectionFailure?.let {
          " Reflection fallback failed with ${it::class.java.simpleName}: ${it.message}."
        } ?: ""
