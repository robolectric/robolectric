...
class RobolectricEnvironment(
  ...
  private val sourceClassLoader: ClassLoader? = null,
  private val classLoadingBridge: FrameworkClassLoadingBridge = DefaultFrameworkClassLoadingBridge,
) {
    ...
    return access {
      if (environmentSetUp) {
        ...
        try {
          ...
          // Only tear down if setup actually succeeded — if setupApplicationState threw,
          // environmentSetUp stays false and the state is not safe to tear down.
          if (environmentSetUp) tearDownApplicationState()
        }
      }
    }
  }
  ...
  fun <T> access(block: () -> T): T {
    ...
    try {
      ...
      // Bridge context makes framework-transformed bytes visible to SandboxClassLoader, while the
      // sandbox classloader still defines/instruments classes.
      return FrameworkClassLoadingContext.withContext(classLoadingBridge, sourceClassLoader) {
        block()
      }
    } finally {
