 ...
 * @property methodHandleDecorators Plugin-loaded decorators applied to shadow method handles
 */
...
class SandboxConfigurator(
  ...
  private val methodHandleDecorators: List<MethodHandleDecorator> = emptyList(),
) {
  /**
  ...
  fun configureSandbox(sandbox: AndroidSandbox, config: Config, sdk: Sdk) {
    ...
    val interceptors = Interceptors(AndroidInterceptors.all())
    val classHandler =
      classHandlerBuilder.build(shadowMap, shadowMatcher, interceptors, methodHandleDecorators)
