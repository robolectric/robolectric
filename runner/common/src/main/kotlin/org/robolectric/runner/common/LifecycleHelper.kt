...
object LifecycleHelper {
  ...
  fun invokeStaticLifecycleMethods(bootstrappedTestClass: Class<*>, annotationClass: Class<*>) {
    invokeCollectedLifecycleMethods(
  ...
  private fun invokeCollectedLifecycleMethods(
    bootstrappedTestClass: Class<*>,
    annotationClass: Class<*>,
    static: Boolean,
    target: Any?,
  ) {
    // Collect lifecycle methods from the class hierarchy, subclass-first. Overridden methods are
    // recorded once (for the subclass) via their signature.
    ...
    while (currentClass != null && currentClass != Object::class.java) {
      currentClass.declaredMethods
        .filter { method -> Modifier.isStatic(method.modifiers) == static }
        .filter { method -> hasLifecycleAnnotation(method, annotationClass) }
        .forEach { method ->
          if (seenSignatures.add(methodSignature(method))) {
  ...
  /**
   * Returns true if the class (or any superclass up to but excluding `Object`) declares at least
   * one method annotated with any of the given `annotationClasses`.
   *
   * Used by test engines to decide whether a class needs `@BeforeAll`/`@AfterAll`-style class
   * lifecycle orchestration. Both JUnit 4 (`@BeforeClass`/`@AfterClass`) and JUnit Jupiter
   * (`@BeforeAll`/`@AfterAll`) can be supplied together by unioning the annotation lists — see
   * [LifecycleAnnotations.JUNIT_COMBINED].
   */
  @JvmStatic
  fun hasLifecycleMethods(
    testClass: Class<*>,
    annotationClasses: List<Class<out Annotation>>,
  ): Boolean {
    if (annotationClasses.isEmpty()) return false
    return generateSequence<Class<*>>(testClass) { it.superclass }
      .takeWhile { it != Any::class.java }
      .any { clazz ->
        clazz.declaredMethods.any { method ->
          annotationClasses.any { method.isAnnotationPresent(it) }
        }
      }
  }
