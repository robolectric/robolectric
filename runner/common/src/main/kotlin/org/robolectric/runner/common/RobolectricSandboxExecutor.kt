        ...
        Callable<Void?> {
          lifecycleManager.executeInSandbox(sandboxContext, testName) {
            TestMethodInvoker.invoke(
              ...
              testClass = testClass,
              testMethod = testMethod,
              beforeEachAnnotations = beforeEachAnnotations,
              afterEachAnnotations = afterEachAnnotations,
              parameterResolver = parameterResolver,
            )
            // Allow callers to attach extra logic (e.g. assertions that depend on context)
