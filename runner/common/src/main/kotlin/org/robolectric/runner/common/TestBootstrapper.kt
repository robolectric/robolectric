...
object TestBootstrapper {
  ...
  fun createTestInstance(bootstrappedClass: Class<*>): Any {
    ...
    val isNonStaticInner = enclosing != null && !Modifier.isStatic(bootstrappedClass.modifiers)
  ...
  fun invokeTestMethod(testInstance: Any, method: Method, args: Array<Any?>): Any? {
    ...
    val bootstrappedParameterTypes =
      method.parameterTypes.map { bootstrappedClass.classLoader.loadClass(it.name) }.toTypedArray()
    val bootstrappedMethod = bootstrappedClass.getMethod(method.name, *bootstrappedParameterTypes)
