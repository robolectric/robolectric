...
object DiscoveryHelpers {
  ...
  /**
   * Default set of @Test annotation FQCNs recognized by [isTestMethod] when no custom set is
   * supplied. Kept as a public constant so non-JUnit framework adapters (Kotest-style,
   * TestNG-style) can union their own annotation names with this default:
   * ```kotlin
   * val myNames = DiscoveryHelpers.DEFAULT_TEST_ANNOTATION_NAMES + "io.kotest.core.annotation.Test"
   * DiscoveryHelpers.isTestMethod(method, myNames)
   * ```
   */
  @JvmField
  val DEFAULT_TEST_ANNOTATION_NAMES: Set<String> =
    setOf("org.junit.jupiter.api.Test", "org.junit.Test")

  /**
   * Determines if a method is a test method by checking for any of the annotation class names in
   * [annotationNames]. Defaults to [DEFAULT_TEST_ANNOTATION_NAMES] (JUnit 4 + Jupiter).
   *
   * Matching is done by fully-qualified annotation class name so the test method check works across
   * classloader boundaries — framework-transformed classes can still be recognized.
   *
   * @param method The method to check
   * @param annotationNames Fully-qualified annotation class names to treat as "@Test"
   * @return true if the method has any of the recognized test annotations
   */
  @JvmStatic
  @JvmOverloads
  fun isTestMethod(
    method: Method,
    annotationNames: Set<String> = DEFAULT_TEST_ANNOTATION_NAMES,
  ): Boolean {
    return method.annotations.any { it.annotationClass.java.name in annotationNames }
  }
