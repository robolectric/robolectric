...
class DefaultRobolectricIntegration(
  ...
  // Per-test mapped SDK levels for PER_SDK strategy
            ...
            if (globalContext == null) {
              globalContext =
                lifecycleManager.createSandbox(
                  testClass,
                  sourceClassLoader = resolveSourceClassLoader(testClass),
                )
  ...
  override fun beforeTest(testClass: Class<*>, testMethod: Method) {
    ...
    when (sandboxSharing) {
      ...
      SandboxSharingStrategy.PER_SDK -> {
        val existingSdkLevel = testSdkLevels[testKey]
        ...
        testSdkLevels[testKey] = sdkLevel
  ...
  override fun afterTest(testClass: Class<*>, testMethod: Method, success: Boolean) {
    ...
    when (sandboxSharing) {
      ...
      SandboxSharingStrategy.PER_SDK,
      SandboxSharingStrategy.GLOBAL -> {
        if (sandboxSharing == SandboxSharingStrategy.PER_SDK) {
          testSdkLevels.remove(testKey)
        }
  ...
  private fun getSandboxContext(
    ...
    return when (sandboxSharing) {
      ...
      SandboxSharingStrategy.PER_SDK -> {
        if (testMethod != null) {
          val testKey = "${testClass.name}.${testMethod.name}"
          val sdkLevel = testSdkLevels[testKey]
          if (sdkLevel != null) {
            sdkContexts[sdkLevel]
          } else {
            val tempContext =
              lifecycleManager.createSandbox(
                testClass,
            ...
            testSdkLevels[testKey] = tempContext.sdk.apiLevel
  ...
  private fun resolveSourceClassLoader(testClass: Class<*>): ClassLoader? {
    return dependencies.frameworkClassLoadingBridge.resolveSourceClassLoader(testClass)
  }
}
